"""dHybridR Trubulence simulations"""
# !==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==
# >-|===|>                                    Imports                                     <|===|-<
# !==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==!==
...
def field_dot(A: ndarray, B: ndarray) -> ndarray:
    """docstring"""
    return nsum(A * B, axis=0)
...
def non_thermal_slope(E, f, mach):
    ...
    E = E[fitting_zone]
    f = f[fitting_zone]
    [slope, b], _ = curve_fit(line, log10(E), log10(f/E))
    return slope, b

def S(p: int, u: ndarray, l: int):
    """
...
class TurbInit(dHybridRinitializer):
    """the initializer for turbulence simulations"""
    def __init__(
        self,
        simulation
        ):
        ...
        # this gives us L, N's, shape, and d's, as well as the basic code to produce B and
        # u fields and save those as d files
    ...
    def configure(self):
        ...
        match self.dims:
            ...
            case 2:
                ...
                # Default value if config file has no kinit
                if "kinit" not in self.config.params: self.kinit = (1, pi), (1, pi)
                else: #if the config file has a kinit value use that
                    if len(self.config.kinit)==2:
                        self.kinit = self.config.kinit, self.config.kinit
                    elif len(self.config.kinit)==4:
                        self.kinit = self.config.kinit[:2], self.config.kinit[2:]
                    else:
                        raise ValueError(
                            f"config file's kinit value is invalid\nkinit={self.config.kinit}"
                            )
                ...
                if not self.simulation.compressed:
                    self.simulation.tau = self.simulation.time * max(
                        self.mach if isinstance(self.mach, Iterable) else [self.mach]
                        ) / (
                            max(self.input.boxsize)
                            )
            ...
            case 3:
                # set the initial k space annuli for producing turbulence
                # Default value if config file has no kinit
                if "kinit" not in self.config.params:
                    self.kinit = (1, 2*pi), (1, 2*pi), (1, 2*pi)
                else: #if the config file has a kinit value use that
                    if len(self.config.kinit)==2:
                        self.kinit = self.config.kinit, self.config.kinit, self.config.kinit
                    # if there are two sets then assume it goes
                    # perp, par -> kinit_perp, kinit_perp, kinit_par
                    elif len(self.config.kinit)==4:
                        self.kinit = (
                            self.config.kinit[:2], self.config.kinit[:2], self.config.kinit[2:]
                            )
                    elif len(self.config.kinit)==6:
                        self.kinit = (
                            self.config.kinit[:2], self.config.kinit[2:4], self.config.kinit[4:]
                            )
                    else:
                        raise ValueError(
                            f"config file's kinit value is invalid\nkinit={self.config.kinit}"
                ...
                # set k range
                self.kmin = 2 * pi / max(self.L)
                # is this even used for anything? I don't think so?
                self.kmax = 2 * pi / min([self.dx, self.dy, self.dz])
                ...
                if "peak_jz" in self.config.params:
                    ...
                    self.simulation.peak = dHybridRSnapshot(
                        self.simulation, self.simulation.peak_jz_ind
                        )
                    ...
                    dHybridRSnapshot(
                        self.simulation,
                        argmin(abs(self.simulation.tau - n))) \
                    for n in range(1, int(self.simulation.tau[-1]//1))]

    def fluctuate3D(self, field, amp, no_div=True):
        """docstring"""
        init_mask = array([
            where((
                self.kinit[0][0] * self.kmin < self.kmag
                )&(
                self.kmag < self.kinit[0][1]*self.kmin),
                True,False),
            where((
                self.kinit[1][0] * self.kmin < self.kmag
                )&(
                self.kmag < self.kinit[1][1]*self.kmin),
                True,False),
            where((
                self.kinit[2][0] * self.kmin < self.kmag
                )&(
                self.kmag < self.kinit[2][1]*self.kmin),
                True,False)
        ])
        M = sum(init_mask)
        ...
        FT[1, :self.Ny // 2, :self.Nx // 2, :self.Nz // 2] = conj(
            _fx[:self.Ny // 2, :self.Nx // 2, :self.Nz // 2])
        FT[1, self.Ny // 2:, :self.Nx // 2, :self.Nz // 2] = conj(
            _fx[self.Ny // 2:, :self.Nx // 2, :self.Nz // 2])
        FT[1, :self.Ny // 2, :self.Nx // 2, self.Nz // 2:] = conj(
            _fx[:self.Ny // 2, :self.Nx // 2, self.Nz // 2:])
        ...
        FT[0, :self.Ny // 2, :self.Nx // 2, :self.Nz // 2] = conj(
            _fy[:self.Ny // 2, :self.Nx // 2, :self.Nz // 2])
        FT[0, :self.Ny // 2, :self.Nx // 2, :self.Nz // 2] = conj(
            _fy[:self.Ny // 2, :self.Nx // 2, :self.Nz // 2])
        FT[0, self.Ny // 2:, :self.Nx // 2, :self.Nz // 2] = conj(
            _fy[self.Ny // 2:, :self.Nx // 2, :self.Nz // 2])
        ...
        FT[2, :self.Ny // 2, :self.Nx // 2, :self.Nz // 2] = conj(
            _fz[:self.Ny // 2, :self.Nx // 2, :self.Nz // 2])
        FT[2, :self.Ny // 2, :self.Nx // 2, :self.Nz // 2] = conj(
            _fz[:self.Ny // 2, :self.Nx // 2, :self.Nz // 2])
        FT[2, self.Ny // 2:, :self.Nx // 2, :self.Nz // 2] = conj(
            _fz[self.Ny // 2:, :self.Nx // 2, :self.Nz // 2])
    ...
    def fluctuate2D(self, field, amp, no_div=True):
        """
        Given the initialization create a 2d array the same shape as the simulation which 
        will smoothly fluctuate over length scales kinit.
        :return y: ndarray[float32]: random fluctuations set by parameters passed to __init__
        """
        init_mask: ndarray = where(
            (self.kinit[0][0] * self.kmin < self.kmag
            ) & (
            self.kmag < self.kinit[0][1] * self.kmin))
    ...
    def construct_field(self, x, y, z, amp, no_div=True):
        """
        Constructs a 3 x N x N array representing a constant x, y, and z component with 
        additional fluctuations
        :param x:
        :param y:
        :param z:
        :param no_div: whether or not to ensure that the divergence of the field is 0 when 
        applying fluctuations
        :return field:
        """
        base_field = array([
            zeros(self.shape) + y,
    ...
    def build_B_field(self):
        """docstring"""
        self.B = self.construct_field(0, 0, 1, self.amplitude[0])

    def build_u_field(self):
        """docstring"""
        self.u = self.construct_field(0, 0, 0, self.amplitude[1])


class TurbSim(dHybridR):
    """dHybridR object for turbulence simulations"""
    def __init__(
            self,
            path: str,
            caching: bool = False,
        ...
        dHybridR.__init__(self,
            ...
            compressed=compressed, debug=debug
        )
...
class TurbGroup(SimulationGroup):
    """a group of turbulence simulations"""
